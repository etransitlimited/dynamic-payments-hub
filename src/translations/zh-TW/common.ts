
const common = {
  "months": {
    "jan": "一月",
    "feb": "二月",
    "mar": "三月",
    "apr": "四月",
    "may": "五月",
    "may_short": "五月",
    "jun": "六月",
    "jul": "七月",
    "aug": "八月",
    "sep": "九月",
    "oct": "十月",
    "nov": "十一月",
    "dec": "十二月"
  },
  "thisMonth": "本月",
  "lastMonth": "上個月",
  "thisWeek": "本週",
  "lastWeek": "上週",
  "comparedToLastWeek": "與上週相比",
  "comparedToLastMonth": "與上月相比",
  "activeUsers": "活躍用戶",
  "newUsers": "新用戶",
  "premiumUsers": "高級用戶",
  "adminRoles": "管理員角色",
  "inactiveUsers": "非活躍用戶",
  "userManagement": "用戶管理",
  "manageUsers": "管理用戶",
  "userManagementDesc": "管理用戶賬戶和訪問級別",
  "depositCompletion": "完成度",
  "viewCards": "查看卡片",
  "viewDeposits": "查看存款",
  "accountManagement": "賬戶管理",
  "accountManagement.title": "賬戶管理",
  "cardManagement": "卡片管理",
  "walletManagement": "錢包管理",
  "depositManagement": "存款管理",
  "cardManagementDesc": "管理卡片賬戶和設置",
  "walletManagementDesc": "管理錢包和交易記錄",
  "activeCards": "活躍卡片",
  "pendingCards": "待處理卡片",
  "processedDeposits": "已處理",
  "pendingDeposits": "待處理",
  "roleManagement": "角色管理",
  "roleManagementDesc": "管理角色和權限",
  "permissionSettings": "權限設置",
  "adminRole": "管理員角色",
  "staffRole": "員工角色",
  "managerRole": "經理角色",
  "admin": "管理員",
  "manager": "經理",
  "editor": "編輯",
  "viewer": "瀏覽者",
  "totalUsers": "總用戶數",
  "activeUsersCount": "活躍用戶",
  "inactiveUsersCount": "非活躍用戶",
  "totalCards": "總卡片數",
  "creditCards": "信用卡",
  "debitCards": "借記卡",
  // Cards types
  "cards.types.credit": "信用卡",
  "cards.types.debit": "借記卡",
  // Account roles
  "accountRoles.cardAccessManagement": "卡片訪問管理",
  "accountRoles.cardAccessDesc": "管理卡片訪問權限",
  "accountRoles.cardActivation": "卡片激活",
  "accountRoles.adminRole": "管理員角色",
  "accountRoles.managerRole": "經理角色",
  "accountRoles.staffRole": "員工角色",
  "accountRoles.depositManagement": "存款管理",
  "accountRoles.cardSettings": "卡片設置",
  "accountRoles.fullAccess": "完全訪問權限",
  "accountRoles.limitedAccess": "有限訪問權限",
  "accountRoles.basicAccess": "基本訪問權限",
  "accountRoles.dashboardAccess": "儀表板訪問",
  "accountRoles.userManagement": "用戶管理",
  "accountRoles.transactionManagement": "交易管理",
  "accountRoles.allRoles": "所有角色",
  "accountRoles.adminOnly": "僅管理員",
  "accountRoles.adminAndManager": "管理員和經理",
  // New translations for permission and user management items
  "permissionsSettings": "權限設置",
  // Wallet and deposit related translations
  "wallet.overview.activeUsers": "活躍用戶",
  // Card statistics
  "cardStatistics.title": "卡片統計",
  "cardStatistics.totalCards": "總卡片數",
  "cardStatistics.activeCards": "活躍卡片",
  "cardStatistics.inactiveCards": "非活躍卡片",
  // Page titles and descriptions 
  "pageTitle.dashboard": "儀表板",
  "pageTitle.accountManagement": "賬戶管理",
  "pageTitle.userManagement": "用戶管理",
  "pageTitle.cardManagement": "卡片管理",
  "pageTitle.walletManagement": "錢包管理",
  "pageTitle.depositManagement": "存款管理",
  "pageTitle.roleManagement": "角色管理"
};

export default common;

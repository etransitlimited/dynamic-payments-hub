...
const InvitationRecordsCard = ({ 
  ...
  return (
    <Card className="bg-gradient-to-br from-[rgb(142,45,226)] to-[rgb(74,0,224)] border-purple-900/50 shadow-lg shadow-purple-900/10 hover:shadow-[0_0_15px_rgba(142,45,226,0.15)] transition-all duration-300 overflow-hidden w-full">
      ...
      <CardContent className="relative z-10 p-6">
          ...
          <div className="relative flex gap-2 w-full max-w-sm">
            <div className="relative flex-1">
              ...
              <Input 
                ...
                className="pl-10 bg-[#061428] border-purple-900/50 text-white placeholder-purple-300/40 w-full"
                value={searchQuery}
        ...
        <div className="rounded-md border border-purple-900/50 overflow-hidden">
          <div className="overflow-x-auto">
            <Table>
              <TableCaption className="text-purple-200/50">{t("invitation.inviteeList")}</TableCaption>
              <TableHeader>
                <TableRow className="border-purple-900/50 hover:bg-transparent">
              ...
              <TableBody>
                {isLoading ? (
                  Array(3).fill(0).map((_, index) => (
                    <TableRow key={`skeleton-${index}`} className="border-purple-900/50">
                      <TableCell><Skeleton className="h-4 w-24 bg-purple-900/20" /></TableCell>
                      <TableCell><Skeleton className="h-4 w-32 bg-purple-900/20" /></TableCell>
                      <TableCell><Skeleton className="h-6 w-16 bg-purple-900/20 rounded-sm" /></TableCell>
                      <TableCell><Skeleton className="h-4 w-12 bg-purple-900/20" /></TableCell>
                      <TableCell><Skeleton className="h-4 w-16 bg-purple-900/20" /></TableCell>
                    </TableRow>
                  ))
                ) : filteredInvitees.length > 0 ? (
                  filteredInvitees.map((invitee, index) => (
                    <TableRow key={index} className="border-purple-900/50 hover:bg-purple-900/20">
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-purple-300 py-8">
                      {t("common.noData")}
                    </TableCell>
                  </TableRow>
                )}
